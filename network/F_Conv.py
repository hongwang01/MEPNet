# -*- coding: utf-8 -*-
"""
refere to https://github.com/XieQi2015/F-Conv/blob/main/SRExp/src/model/F_Conv.py
"""
import numpy as np
import torch
...
class F_BN(nn.Module):
    def __init__(self,channels, tranNum=8):
        ...
        self.tranNum = tranNum
        # self.scale = nn.Parameter(torch.ones(1, 4, 1, 1), requires_grad=True)   # Fu
        # self.bias = nn.Parameter(torch.zeros(1, 4, 1, 1), requires_grad=True)    # Fu
    def forward(self, X):
        X = self.BN(X.reshape([X.size(0), int(X.size(1)/self.tranNum), self.tranNum*X.size(2), X.size(3)]))
        # X = X*self.scale + self.bias    # Fu
